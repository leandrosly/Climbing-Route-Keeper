...
### App: WebView (não Flutter, não Capacitor — por enquanto)
- Decisão consciente de começar simples: o app mobile é essencialmente um WebView exibindo a aplicação web.
- Trade-off conhecido e aceito: acesso a hardware (câmera) fica mais limitado que numa solução nativa ou via Capacitor — mas isso é mitigado pelo fluxo de câmera escolhido (ver abaixo).
- Pode ser revisitado no futuro se as limitações do WebView pesarem na prática.
...
- Motivo: mais simples de implementar (é só um seletor de arquivo `<input type="file">`, funciona bem em qualquer WebView, sem precisar de `getUserMedia`), e aproveita o processamento superior da câmera nativa do celular (HDR, foco, resolução).
...
- Sem uso de nuvem (AWS) por enquanto — prioridade é manter custo baixo/zero enquanto o projeto está em fase de validação.
